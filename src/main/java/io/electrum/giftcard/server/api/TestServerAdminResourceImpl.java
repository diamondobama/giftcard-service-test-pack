...
public class TestServerAdminResourceImpl {
   ...
   @ApiOperation(value = "Retrieve the test data configured in the Giftcard Test Server for a certain card.", notes = "The Test "
         ...
         + "in a test.", authorizations = { @Authorization(value = "httpBasic") }, tags = { "Test Server Admin", })
   @ApiResponses(value = { @ApiResponse(code = 200, message = "OK", response = DataResponse.class),
         @ApiResponse(code = 500, message = "Internal Server Error") })
   ...
   public void deleteCardData(
         @ApiParam(value = "The PAN of the card for which data should be deleted.", required = true) @PathParam("cardNumber") String cardNumber,
         @Context SecurityContext securityContext,
         @Suspended AsyncResponse asyncResponse,
         @Context HttpHeaders httpHeaders,
         @Context UriInfo uriInfo,
   ...
   @Consumes({ "application/json" })
   @Produces({ "application/json" })
   @ApiOperation(value = "Add one or more cards to the test database.", notes = "The Test "
         + "Server Admin Data endpoint allows a user of the Test Server to add "
         + "cards to the test database backing the Test Server. In this way a "
         + "user can provide their own card information to use for testing.", authorizations = {
               @Authorization(value = "httpBasic") }, tags = { "Test Server Admin", })
   ...
   public void addCardData(
      ...
      log.info(String.format("%s %s", httpServletRequest.getMethod(), uriInfo.getPath()));
      Response rsp =
            GiftcardMessageHandlerFactory.getDataHandler().handleAddCardDataRequest(cards, httpHeaders, uriInfo);
      log.debug(String.format("Entity returned:\n%s", rsp.getEntity()));
      asyncResponse.resume(rsp);
   }
